...
exports.recomputeDashboardCounters = exports.onHazardReportStatusChangeDenorm = exports.onHazardReportCreatedDenorm = exports.dailyResetTodayChecklistFlag = exports.onChecklistSubmittedDenorm = void 0;
/**
 ...
 */
exports.onChecklistSubmittedDenorm = (0, firestore_1.onDocumentWritten)("checklists/{checklistId}", async (event) => {
    var _a;
    ...
    console.log(`[denorm/checklistDone] uid=${uid} flag=true date=${after.date}`);
});
/**
 ...
 */
exports.dailyResetTodayChecklistFlag = (0, scheduler_1.onSchedule)({ schedule: "5 0 * * *", timeZone: "UTC", region: "us-central1" }, async () => {
 ...
 */
exports.onHazardReportCreatedDenorm = (0, firestore_1.onDocumentCreated)("hazard_reports/{reportId}", async (event) => {
    var _a, _b;
    ...
    if (!OPEN_STATUSES.has(status))
        return;
    await admin.firestore().collection("users").doc(uid).set({ pendingReportCount: admin.firestore.FieldValue.increment(1) }, { merge: true });
 ...
 */
exports.onHazardReportStatusChangeDenorm = (0, firestore_1.onDocumentUpdated)("hazard_reports/{reportId}", async (event) => {
    var _a, _b;
    ...
    if (delta === 0)
        return;
    await admin.firestore().collection("users").doc(uid).set({ pendingReportCount: admin.firestore.FieldValue.increment(delta) }, { merge: true });
    console.log(`[denorm/reportStatus] uid=${uid} ${before.status}→${after.status} delta=${delta}`);
});
/**
 ...
 */
exports.recomputeDashboardCounters = (0, scheduler_1.onSchedule)(
// Disabled schedule (year=2099) — only triggered manually.
{ schedule: "0 0 1 1 *", timeZone: "UTC", region: "us-central1" }, async () => {
    var _a, _b;
    const db = admin.firestore();
    ...
    for (const doc of reportsSnap.docs) {
        ...
        if (!uid)
            continue;
        pendingByUid.set(uid, ((_a = pendingByUid.get(uid)) !== null && _a !== void 0 ? _a : 0) + 1);
    }
    ...
    const checklistsSnap = await db
        ...
        .get();
    const submittedToday = new Set(checklistsSnap.docs
        .map((d) => d.data().uid)
        .filter((u) => !!u));
    ...
    for (let i = 0; i < usersSnap.docs.length; i += 400) {
        ...
        for (const doc of chunk) {
            batch.update(doc.ref, {
                pendingReportCount: (_b = pendingByUid.get(doc.id)) !== null && _b !== void 0 ? _b : 0,
...
//# sourceMappingURL=denormaliseDashboardCounters.js.map